...
function showHikeSingle(hikename) {
    document.getElementsByClassName('hikeBox').classList.add('hidden');
    document.getElementById(hikename).classList.remove('hidden');

}

function renderOneHikeLight(hike) {
    const item = document.createElement("li");
    const id = hike.name.split(" ").join("").toLowerCase();
    item.innerHTML = ` <h2>${hike.name}</h2>
    <div id=${id} class="hikeBox" onclick="showHikeSingle(${id})"> 
    <div class="image">
